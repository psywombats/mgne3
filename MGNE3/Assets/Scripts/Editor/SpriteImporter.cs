...
internal sealed class SpriteImporter : AssetPostprocessor {
    ...
    public static void CreateAnimations(string assetPath) {
        ...
        if (path.Contains("Charas")) {
            ...
            for (int i = 0; i < 4; i += 1) {
                AnimationClip originalClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(facingsDir + "Placeholder/Placeholder" + FacingNames[i] + ".anim");
                AnimationClip overrideClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(facingsDir + name + "/" + name + FacingNames[i] + ".anim");
                clips.Add(new KeyValuePair<AnimationClip, AnimationClip>(originalClip, overrideClip));
            }
            for (int i = 0; i < 4; i += 1) {
                AnimationClip originalClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(facingsDir + "Placeholder/Placeholder" + FacingNames[i] + "Idle.anim");
                AnimationClip overrideClip;
                if (stepCount > 2) {
                    overrideClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(facingsDir + name + "/" + name + FacingNames[i] + "Idle.anim");
                } else {
                    overrideClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(facingsDir + name + "/" + name + FacingNames[i] + ".anim");
                }
                clips.Add(new KeyValuePair<AnimationClip, AnimationClip>(originalClip, overrideClip));
            }
